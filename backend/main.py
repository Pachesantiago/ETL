"""
ETL Principal para OtakuLATAM
Orquesta todo el proceso de Extract, Transform, Load
"""
import os
import sys
import logging
...
class OtakuETL:
    """Clase principal del ETL de OtakuLATAM"""
    ...
    def initialize_services(self):
        ...
        try:
            self.logger.info("Inicializando servicios externos")
    ...
    def run_etl(self, input_file_path=None):
        """
        Ejecuta el proceso completo del ETL
        ...
        try:
            ...
            self.logger.info("="*60)
            self.logger.info("INICIANDO ETL OTAKU LATAM")
            self.logger.info("="*60)
            ...
            transformed_data, transformation_result = self._transform_phase(extracted_data)
            
            # FASE 3: CARGA
            self.logger.info("FASE 3: CARGA DE DATOS")
            load_result = self._load_phase(transformed_data)
            ...
            self.logger.error(f"Error en ejecución del ETL: {e}")
    ...
    def _transform_phase(self, extracted_data):
        ...
        try:
            ...
            self.logger.info(f"Transformación completada: {len(transformed_data)} registros")
            
            # Log en BD
            if self.db_connection and self.ejecucion_id:
                self.db_connection.registrar_log_etl(
                    self.ejecucion_id, 'INFO', 
                    f"Transformación completada: {len(transformed_data)} registros",
                    {
            ...
            self.logger.error(f"Error en fase de transformación: {e}")
            if self.db_connection and self.ejecucion_id:
                self.db_connection.registrar_log_etl(
                    self.ejecucion_id, 'ERROR', 
    ...
    def _load_phase(self, transformed_data):
        ...
        try:
            ...
            # Cargar datos a todos los destinos
            ...
            self.logger.info("Carga de datos completada")
            ...
            self.logger.error(f"Error en fase de carga: {e}")
            if self.db_connection and self.ejecucion_id:
                self.db_connection.registrar_log_etl(
                    self.ejecucion_id, 'ERROR', 
                    f"Error en carga: {str(e)}"
                )
            raise
